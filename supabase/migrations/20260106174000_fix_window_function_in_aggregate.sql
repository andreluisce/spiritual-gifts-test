-- Fix 'aggregate function calls cannot contain window function calls' error
-- Problem: Using ROW_NUMBER() inside jsonb_agg()
-- Solution: Calculate row numbers in CTE first, then aggregate
...
BEGIN
    ...
    SELECT
      ...
      -- Get top 3 gifts with names - FIX: Use CTE to avoid window function in aggregate
      COALESCE(
        (WITH ranked_gifts AS (
            SELECT
              ...
              CASE qrw.gift
                ...
                ELSE qrw.gift::text
              END as gift_name,
              ROW_NUMBER() OVER (ORDER BY qrw.total_weighted DESC) as gift_rank
...
COMMENT ON FUNCTION public.get_user_quiz_results IS 'Returns all quiz results for a specific user (admin only) - Fixed window function error by using CTE';

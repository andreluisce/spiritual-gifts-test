-- Fix get_quiz_report to handle errors more gracefully and provide better error messages
DROP FUNCTION IF EXISTS public.get_quiz_report(UUID);
...
BEGIN
    ...
    IF NOT v_is_admin AND (v_current_user_id IS NULL OR v_user_id != v_current_user_id) THEN
        ...
        'ai_insights', (
            SELECT COALESCE(json_agg(
                json_build_object(
                    ...
                    'description', COALESCE(gc.description, 'No description available'),
                    'biblical_foundation', COALESCE(gc.biblical_foundation, 'No biblical foundation available'),
                    'practical_applications', COALESCE(gc.practical_applications, 'No practical applications available')
                ) ORDER BY rg.rank
            ), '[]'::json)
            FROM (
                SELECT
            ...
            LEFT JOIN gift_content gc ON gc.gift_key = rg.gift::text AND gc.locale = 'pt'
        )
    ) INTO report_data;
...
COMMENT ON FUNCTION public.get_quiz_report IS 'Returns comprehensive quiz report with improved error handling and real data only';

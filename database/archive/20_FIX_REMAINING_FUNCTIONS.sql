...
-- Fix get_recent_activity with proper subquery
...
-- Fix get_gift_distribution to work with actual data
...
BEGIN
    ...
    -- Return gift distribution from quiz_results_weighted
    SELECT json_agg(
        json_build_object(
            'gift_id', gift_id,
            'gift_name', gift_name,
...
GRANT EXECUTE ON FUNCTION public.get_recent_activity(INTEGER) TO anon;
GRANT EXECUTE ON FUNCTION public.get_gift_distribution() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_gift_distribution() TO anon;
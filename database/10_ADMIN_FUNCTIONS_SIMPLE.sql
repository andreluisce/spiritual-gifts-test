-- =========================================================
-- SIMPLIFIED ADMIN FUNCTIONS FOR TESTING (NO AUTH CHECK)
-- =========================================================

-- Function to get user statistics for admin dashboard (simplified)
...
-- Function to get users with stats for admin management (simplified)
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(u.raw_user_meta_data ->> 'name', split_part(u.email, '@', 1), 'Unknown') as user_name,
    'Completed quiz' as action,
    'quiz' as type,
    COALESCE(qs.completed_at, qs.created_at) as created_at
  FROM public.quiz_sessions qs
...
-- Function to get gift distribution (simplified)
...
-- Function to delete a user (simplified)
...
-- Function to update user role (simplified)

...
export default function DashboardPage() {
  ...
  // Only show loading for queries when user is authenticated
  const loading = user ? (loadingResults || loadingLatestResult || authLoading) : authLoading
  ...
  }, [user, authLoading, locale, router, redirecting])
  ...
  // Show loading only when auth is loading or when redirecting
  ...
  // This should not render if user is null (redirect will happen)
  if (!user) {
    return null
  }

  const giftEvolution = getGiftEvolution()
  const showSecondCard = Boolean(giftEvolution) || canTakeQuiz
        ...
        <div className="p-2 sm:p-4 m-1 sm:m-4">
          ...
          {!results || results.length === 0 ? (
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              <div className={`grid grid-cols-1 gap-6 lg:gap-8 ${showSecondCard ? 'xl:grid-cols-2' : ''}`}>
                {/* Latest Results */}
                <Card>
                  <CardHeader>
